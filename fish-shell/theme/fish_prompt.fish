function fish_prompt
  ...
  if not set -q __fish_prompt_hostname
    ...
    set -g __fish_prompt_hostname $turquoise(prompt_hostname)(set_color normal)
  end
  ...
  if [ (id -u) -eq 0 ]
    # top line > Superuser
    echo -n $turquoise$USER$white$location$__fish_prompt_hostname$white' '$purple(pwd)$turquoise
    __fish_git_prompt " (%s)"
    echo
